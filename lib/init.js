...
async function init() {
  ...
  if (detected.existingFiles.length > 0) {
    ...
    console.log('  Scripts (pipeline/scripts/*.sh) are always updated to the latest version.');
    console.log('');
  }
  ...
  console.log(`     ${GREEN}./pipeline/scripts/setup.sh${NC}`);
  console.log('');
  ...
  if (config.runBootstrap) {
    console.log(`  ${step}. Bootstrap docs from your codebase:`);
    console.log(`     ${GREEN}./pipeline/scripts/setup.sh --bootstrap${NC}`);
    console.log('');
    step++;
  }
  console.log(`  ${step}. Generate initial context file:`);
  console.log(`     ${GREEN}./pipeline/scripts/generate-context.sh${NC}`);
  step++;

  console.log('');
  console.log(`  ${step}. Start building:`);
  console.log(`     cp pipeline/specs/_template.md pipeline/specs/my-feature.md`);
  console.log(`     # Edit the spec`);
  console.log(`     ${GREEN}./pipeline/scripts/dev-loop.sh my-feature${NC}`);
  console.log('');
}
