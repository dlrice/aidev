...
function generate(cwd, config, { force = false } = {}) {
  ...
  const dirs = [
    'specs', 'plans', 'docs', 'scripts', 'logs', 'src',
    'reviews/spec-critiques', 'reviews/plan-reviews', 'reviews/adversarial',
    'tests/gemini', 'tests/qwen', 'tests/adversarial', 'tests/merged',
  ];
  ...
  writeIfNew(cwd, 'pipeline.config.json', renderConfig(config), force, written, skipped);
  ...
  writeIfNew(cwd, 'specs/_template.md', readTemplate('specs/_template.md'), force, written, skipped);

  // Generate doc stubs
  writeIfNew(cwd, 'docs/ARCHITECTURE.md', renderArchDocs(ctx), force, written, skipped);
  writeIfNew(cwd, 'docs/CONVENTIONS.md', renderConventionsDocs(ctx), force, written, skipped);
  writeIfNew(cwd, 'docs/DATA-MODEL.md', readTemplate('docs/DATA-MODEL.md'), force, written, skipped);
  writeIfNew(cwd, 'docs/DEPENDENCIES.md', readTemplate('docs/DEPENDENCIES.md'), force, written, skipped);
  writeIfNew(cwd, 'docs/DECISIONS.md', readTemplate('docs/DECISIONS.md'), force, written, skipped);

  // Generate scripts (always update these — they're the pipeline logic)
  writeAlways(cwd, 'scripts/dev-loop.sh', readTemplate('scripts/dev-loop.sh'), written);
  writeAlways(cwd, 'scripts/setup.sh', readTemplate('scripts/setup.sh'), written);
  writeAlways(cwd, 'scripts/generate-context.sh', readTemplate('scripts/generate-context.sh'), written);

  // Make scripts executable
  for (const script of ['scripts/dev-loop.sh', 'scripts/setup.sh', 'scripts/generate-context.sh']) {
    try { fs.chmodSync(path.join(cwd, script), 0o755); } catch (e) { /* windows */ }
  }
...
## What NOT To Do
- Do not modify files in specs/, plans/, or reviews/
...
decomposition to \`specs/{feature-name}-decomposition.md\` instead of proceeding.
...
\`COMPLEXITY_LIMIT: This feature should be split.\` and generate a
decomposition to \`specs/{feature-name}-decomposition.md\` instead of
proceeding.
`;
}
